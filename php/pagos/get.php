...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Verifica que la solicitud sea POST
    ...
    if ($json_data) {
        ...
        if ($data !== null) { // Verifica si la decodificación fue exitosa
            ...
            $stmt = $conn->prepare("SELECT idwl, patente FROM wlParking WHERE idwl = ?"); // Prepara una consulta SQL para obtener registros por ID
    ...
    // De lo contrario, devuelve todos los registros filtrados por fecha
    else {
        // Obtener la fecha si está en los datos enviados
        $fechaFiltrada = isset($_POST['fecha']) ? $_POST['fecha'] : null;

        // Si hay una fecha, se filtra en la consulta
        if ($fechaFiltrada) {
            $stmt = $conn->prepare("SELECT idwl, patente FROM wlParking WHERE fecha_salida = ? ORDER BY idwl");
            $stmt->bind_param("s", $fechaFiltrada); // Filtra por la fecha proporcionada
        } else {
            // Si no hay filtro de fecha, se devuelve todos los registros
            $stmt = $conn->prepare("SELECT idwl, patente FROM wlParking ORDER BY idwl");
        }
